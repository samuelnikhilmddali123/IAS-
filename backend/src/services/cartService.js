...
const addToCart = async (userId, foodId, quantity) => {
    ...
    if (existingItem) {
        ...
        if (newQuantity > food.availableQuantity) {
            const error = new Error(
                `Only ${food.availableQuantity} items available`
            );
            error.statusCode = 400;
            throw error;
        }
    ...
    return await Cart.findById(cart._id)
        .populate("items.food", "name category price availableQuantity isAvailable");
};
const getCart = async (userId) => {
    const cart = await Cart.findOne({ user: userId })
        .populate(
            "items.food",
            "name category price availableQuantity isAvailable"
        );
